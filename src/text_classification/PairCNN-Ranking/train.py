...
tf.flags.DEFINE_string("filter_sizes", "2,3,4,5", "Comma-separated filter sizes (default: '2,3')")
...
# Data Parameter
tf.flags.DEFINE_integer("max_len_left", 20, "max document length of left input")
tf.flags.DEFINE_integer("max_len_right", 20, "max document length of right input")
...
def load_data(filepath, vocab_tuple=None):
    data = list(set(open(filepath, encoding='utf-8').readlines()))
    data = [d.replace('\n', '') for d in data]
    data = [d.split(r',') for d in data]
    # data = filter(lambda x: len(x) == 3, data)
    data_left = [clean_str(x[0]).split(' ') for x in data]
    data_right = [clean_str(x[1]).split(' ') for x in data]
...
def main():
    # Load data
    print("Loading data...")
    x_left_train, x_right_train, y_train, vocab, vocab_inv, num_pos = \
        load_data(os.path.join(FLAGS.train_dir, 'data1\\train.txt'))
    x_left_dev, x_right_dev, y_dev, vocab, vocab_inv, num_pos = \
        load_data(os.path.join(FLAGS.train_dir, 'data1\\test.txt'), (vocab, vocab_inv))
